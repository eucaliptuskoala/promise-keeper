...
def interpret_message(
    ...
    instructions = (
        ...
        "If the commitment depends on another person or task being done first, match it to candidate_dependencies and "
        "MUST set depends_on_promise_id. Dependency cues include 'after', 'before', 'once', 'but first', 'after that', "
        "and Russian equivalents such as 'после', 'сначала', and 'как только'. Do not create an independent commitment "
        "when one of these cues refers to a candidate dependency; if the candidate is ambiguous, ask one concise clarification. "
