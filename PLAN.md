...
`agent.run_agent` drives native Chat Completions function calling through the existing OpenAI SDK and shared `process_event`. `tools.model_tools` derives narrow argument schemas from the existing Pydantic fields. The tools are `create_promise`, `complete_promise`, `reschedule_promise`, `ignore_message` and `ask_clarification`. Confirmation, dismissal and snooze remain deterministic button actions; scheduling does not call the model. No agent framework or extra dependency is introduced.

The first request requires exactly one function call and disables parallel calls. Unknown names, unexpected or missing arguments, invalid JSON, oversized arguments and truncated or multiple calls fail before any write. A valid call maps to the existing `AgentDecision` and passes the pipeline's grounding, scope, lifecycle and transaction checks. After the pipeline returns, the second request receives the actual result under the matching tool-call ID, including explicit success and pending delivery, with further tools disabled. Its prose never replaces a card or clarification. An acknowledgement failure cannot undo a committed operation or rerun it; replayed events do not call the model again.

An event uses at most two finite-timeout requests, with SDK retries disabled. There is no JSON-mode or regex fallback. The selected gateway's authenticated generation, strict function schemas, required/disabled tool selection and context limits still need a live check; no model key was available. Offline tests verify protocol handling and real SQLite effects, not gateway compatibility or model quality.
